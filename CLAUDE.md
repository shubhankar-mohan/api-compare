...
│   ├── errorDiagnostics.ts      # Classifies request failures into actionable kinds + unreachable hints
│   ├── comparisonOutcome.ts     # Toast text from request outcomes (never a body verdict)
│   ├── localhostRequest.ts      # Prod-vs-Local: same request aimed at a local base URL
│   ├── mergeText.ts             # Repairs member commas after a line-level JSON merge
...
│   ├── sequenceAlign.ts         # Shared LCS aligner: prefix/suffix trim, interning, cell budget
...
│   ├── diffProperties.test.ts   # Seeded randomized property suite for the engine
...
- **Size guards are a cell budget, not a line count.** `MAX_LCS_CELLS` / `MAX_ARRAY_LCS_CELLS` (16M cells of `Int32Array` = 64 MB) bound the LCS table. A count-based cap creates an *inverted* cliff — the worst input sits just below the threshold and one line more silently flips to a positional zip that reports a single insertion as a full rewrite (1499 lines: +1/-0 in 82ms; 1500: +751/-750 in 4ms). It also rejected cheap asymmetric inputs like 5000x20. Both paths align through `sequenceAlign.ts`, which trims the common prefix and suffix and interns tokens before building the table, so the budget only applies to what actually differs — a 20000-line document with one insertion never hits it. When the remaining middle exceeds the budget it is zipped positionally and the result sets `degraded: true` (on the array path too — it used to degrade silently) so the UI can say the alignment is approximate rather than presenting it as fact. Perf tests must straddle every guard, not just sit above it.
...
- **Array element identity is a heuristic and needs a fallback.** `ID_KEY_CANDIDATES` misses `user_id`, and when the id field is itself what changed (`name`, `sku`, `code`) it cannot help by definition. `pairAdjacentEdits` + `elementSimilarity` fold an adjacent delete/insert run into in-place edits above `ARRAY_PAIR_THRESHOLD`, mirroring `MODIFIED_PAIR_THRESHOLD` on the text path. Without it a one-field change rendered a 57-line record twice, stacked, with no inline highlight. Pairing within a run is a *monotone* maximum-weight matching (`pairBySimilarity`), not a positional zip — the zip paired `[A, B]` vs `[B', C]` as A↔B' and B↔C and hid the edit to B — and it must stay monotone: a crossing pair would emit the right pane out of its own order and break I1, so a swap correctly shows as a removal plus an insertion.
- **Go to Path resolves `DiffLine.path`, never a regex over the rendered text.** Every row the tree diff emits carries the JSONPath of the value it belongs to (rows inside removed/added/unchanged subtrees included). The old lookup built a RegExp from the last segment of the user's input, so `$.b.id` landed on the first `id` in the document and any key containing `[` or `(` threw from an uncaught handler. Keep the fallback literal search for text diffs only.
- **One diff feeds both the panes and the summary.** `DiffViewer` reports `{hasDifferences, statsSkipped}` for the result it rendered via `onSummary`; `Index.tsx` must not run its own `computeDiff` for the card — it did, without rules or options, so a rule-suppressed field was greyed in the viewer and counted above it.
- **Options in the Diff panel must reach the rendered diff, not just the stats.** (The "Detect reordering" switch and key-field input were removed for exactly this reason: array identity is always by key and they changed nothing on screen.) `semanticComparison`, `ignoreCase` and `ignoreWhitespace` are threaded to `jsonTreeDiff` via `normalizeScalar` and affect comparison only — rendering always shows what the server sent. They previously fed `deepEqual` alone, so the panel changed the percentage while the panes ignored it. They must also reach array element identity (`canonical`, `canonicalMasked`, `deepEquals`, `elementSimilarity`, key-field tokens): for a while they applied only in `walkScalars`, so `{a:"1"}` vs `{a:1}` was equal under semantic comparison while `["1"]` vs `[1]` was a removal plus an addition.
- **Inline highlights are computed on the value span, and similarity on the full string.** `computeScalarRowSegments` diffs only the scalar value; the `"key": ` prefix and comma are emitted as unchanged. Handing the whole rendered line to the text-path inline differ let the shared scaffolding satisfy the similarity threshold, so two unrelated names rendered as scattered single-character matches. `calculateSimilarity` is keyed by the full strings — a sampled key (length + first/last 50 chars) collided for lines that differ only in the middle and returned the wrong pair's score — and lines over the Levenshtein cap are scored on their sampled ends rather than dividing a capped distance by the full length, which made every long line look ≥ 0.85 similar.
- **Noise *suggestions* for numbers are gated on the key.** `autoSuggestion` accepts an integer only under a time-like key (`createdAt`, `expires_at`, `timestamp`, `epoch`, `expiry`) and only for the 10–13 digit epoch shape, so numeric `createdAt` gets a chip while a 13-digit `orderId` stays an id. Suggestions are advisory and still counted; suppression needs a saved rule.
- **`src/lib/diffProperties.test.ts` is the randomized guard.** Seeded and deterministic; it found the `sortKeys:false` comma bug and the `"Infinity"`/`null` collision that the hand-written invariants missed. Keep it under a couple of seconds and add a pinned literal when it finds something.
...
- **Never render response text as HTML.** `JsonSyntaxHighlight` tokenizes a line and renders React elements. It used to build an HTML string (escaping only the tokens it matched, splicing them into the *unescaped* line) and hand it to `dangerouslySetInnerHTML`, so a gateway's HTML error page — identical on both sides, hence an "unchanged" row — executed in the page. There is no `dangerouslySetInnerHTML` in the app; keep it that way.
- **There is one ignore mechanism.** Diff Options "ignore key/path" becomes rule paths (`$..key`) and goes through the same code path as saved noise rules: rendered, greyed, not counted, masked for array identity, labelled `source: 'option'`. It used to delete the members from the documents before diffing, which broke I1, drifted line numbers and made the merge silently drop keys. Do not filter documents.
- **The completion toast reports request outcomes, not a verdict.** `comparisonOutcome` says which side failed or that status codes differ; whether the bodies differ is the diff's verdict via `onSummary`. Comparing raw body strings said "identical" for two failed requests and contradicted the badge on key-order-only changes.
- **cURL is tokenized with shell word semantics.** Adjacent quoted/unquoted runs join into one word (`'{"n":"O'"'"'Brien"}'`), double quotes escape only `` $ ` " \ ``, `^"` caret escaping from Windows cmd is undone, `-G` moves data to the query, `-F` implies POST, glued short options (`-XPUT`, `-H"X: 1"`) split, and a scheme-less URL gets `http://`. Prod-vs-Local builds the localhost request as an object (`toLocalhostRequest`), never by re-serialising to a shell string.
- **Mixed content:** App may be served on HTTPS but users compare against HTTP localhost. Browsers treat loopback as potentially trustworthy, so `errorDiagnostics` classifies `http://localhost` from an HTTPS page as mixed content only on Safari; everywhere else it falls through to the reachability probe. The local proxy sidesteps it entirely.
- **The proxy allow-lists nothing by default.** Only loopback origins may use it; a hosted copy of the app must be named with `--allow-origin`. It used to pre-authorise `https://diffchecker.dev`, a domain this project does not own. Upstream responses are read with a 50 MB budget, request bodies get a 413, and `GET /health` answers without an Origin.
