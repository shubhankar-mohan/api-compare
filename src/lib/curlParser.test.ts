...
describe('curlParser', () => {
  ...
  describe('body parsing', () => {
    ...
    it('parses multiline JSON body (the critical bug case)', () => {
      const curl = `curl --location 'https://api.example.com/v1/widget_products' \\
...
--data '{
  ...
  "widget_id": 3365,
  "widget_type": 56,
  "request_source": "web::3365::default",
  ...
  "data": {
    ...
    "sort_by_rating": true
      ...
      expect(result.url).toBe('https://api.example.com/v1/widget_products');
      expect(result.method).toBe('POST');
      expect(result.headers['content-type']).toBe('application/json');
      expect(result.body).toBeTruthy();
      // Verify body is valid JSON
      const parsed = JSON.parse(result.body!);
      expect(parsed.user_id).toBe('usr_0f3c9a1b7e2d4c6a8b5f');
      expect(parsed.widget_id).toBe(3365);
      expect(parsed.data.limit).toBe(3);
      expect(parsed.data.seller).toBe('acme');
    });
